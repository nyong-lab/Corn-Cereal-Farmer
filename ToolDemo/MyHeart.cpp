...
MyHeart::MyHeart(int id, std::string name, Vector2 position, Vector2 size)
...
void MyHeart::Move(Vector2 vec)
{
	m_Position.x = vec.x;
	m_Position.y = vec.y;
}

void MyHeart::Render(D2DEngine1* d2dEngine)
{
	int width = 0;
	int height = 0;
	m_WindowID = std::to_string(m_ID) + m_Name;
	d2dEngine->GetInstance()->LoadTextureFromFile("..\\Resources\\Image\\ImGUI\\heart.png", &m_Texture, &width, &height);
